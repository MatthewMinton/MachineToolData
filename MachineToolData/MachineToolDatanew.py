...
class ToolDialog:
    ...
    def __init__(self, parent, title, initial_data=None):
        ...
        # Tool ID
        tk.Label(main_frame, text="Tool ID:", font=("Arial", 12, "bold"), 
                bg="#2b2b2b", fg="white").grid(row=0, column=0, sticky="w", pady=5)
        self.tool_id_entry = tk.Entry(main_frame, width=30, font=("Arial", 11),
                                     bg="#404040", fg="white", insertbackground="white")
        self.tool_id_entry.grid(row=0, column=1, pady=5, padx=(10, 0), sticky="ew")
        
        # Description
        tk.Label(main_frame, text="Description:", font=("Arial", 12, "bold"), 
                bg="#2b2b2b", fg="white").grid(row=1, column=0, sticky="w", pady=5)
        self.description_entry = tk.Entry(main_frame, width=30, font=("Arial", 11),
                                         bg="#404040", fg="white", insertbackground="white")
        self.description_entry.grid(row=1, column=1, pady=5, padx=(10, 0), sticky="ew")
        
        # PTT Setting
        tk.Label(main_frame, text="PTT (inch):", font=("Arial", 12, "bold"), 
                bg="#2b2b2b", fg="white").grid(row=2, column=0, sticky="w", pady=5)
        self.ptt_entry = tk.Entry(main_frame, width=30, font=("Arial", 11),
                                 bg="#404040", fg="white", insertbackground="white")
        self.ptt_entry.grid(row=2, column=1, pady=5, padx=(10, 0), sticky="ew")
        
        # Height
        tk.Label(main_frame, text="Height (mm):", font=("Arial", 12, "bold"), 
                bg="#2b2b2b", fg="white").grid(row=3, column=0, sticky="w", pady=5)
        self.height_entry = tk.Entry(main_frame, width=30, font=("Arial", 11),
                                    bg="#404040", fg="white", insertbackground="white")
        self.height_entry.grid(row=3, column=1, pady=5, padx=(10, 0), sticky="ew")
        
        # Last sharpened
        tk.Label(main_frame, text="Last Sharpened:", font=("Arial", 12, "bold"), 
        ...
        today_btn = tk.Button(date_frame, text="Today", 
                             command=self.set_today_date, bg="#2196F3", fg="white", 
        ...
        if initial_data:
            ...
            self.ptt_entry.insert(0, str(initial_data.get("ptt_inch", "")))
        ...
        save_btn = tk.Button(btn_frame, text="Save", command=self.save,
                            bg="#4CAF50", fg="white", width=10, font=("Arial", 11, "bold"))
        save_btn.pack(side="left", padx=5)
        
        cancel_btn = tk.Button(btn_frame, text="Cancel", command=self.cancel,
                              bg="#9E9E9E", fg="white", width=10, font=("Arial", 11, "bold"))
    ...
    def save(self):
        ...
        # Validate PTT if provided
        ptt_str = self.ptt_entry.get().strip()
        ptt_mm = ""
        if ptt_str:
            try:
                ptt_inch = float(ptt_str)
                ptt_mm = str(round(ptt_inch * 25.4, 3))
            except ValueError:
                messagebox.showerror("Validation Error", "PTT must be a valid number")
                self.ptt_entry.focus()
                return
...
class ChangePasswordDialog:
    ...
    def __init__(self, parent, username, hash_func):
        ...
        tk.Label(main_frame, text=f"Change password for '{username}'", 
                font=("Arial", 12, "bold"), bg="#2b2b2b", fg="white").grid(row=0, column=0, columnspan=2, pady=(0, 15))

        tk.Label(main_frame, text="New Password:", bg="#2b2b2b", fg="white").grid(row=1, column=0, sticky="w", pady=5)
        self.new_pwd_entry = tk.Entry(main_frame, width=25, show="*", font=("Arial", 11),
                                     bg="#404040", fg="white", insertbackground="white")
        self.new_pwd_entry.grid(row=1, column=1, pady=5, padx=(10, 0))

        tk.Label(main_frame, text="Confirm Password:", bg="#2b2b2b", fg="white").grid(row=2, column=0, sticky="w", pady=5)
        self.confirm_entry = tk.Entry(main_frame, width=25, show="*", font=("Arial", 11),
                                     bg="#404040", fg="white", insertbackground="white")
...
class MachineToolManager:
    def __init__(self):
        ...
        # Configuration - Updated machine list
        self.machines = [
            "5000-1", "5000-2", "5000-3", "5000-4", 
            "6000-1", "6000-2", "6000-3", 
            "7000-1", "7000-2",
            "5030-1", "5030-2"
        ]
        
        # Machines that have lens change functionality
        self.lens_machines = ["6000-1", "6000-2", "6000-3", "7000-1", "7000-2", "5030-1", "5030-2"]
        
        # Machines that are lens-only (no tools)
        self.lens_only_machines = ["5030-1", "5030-2"]
    ...
    def show_login_screen(self):
        ...
        login_frame = tk.Frame(main_frame, bg="#2b2b2b", relief="raised", bd=2)
        login_frame.place(relx=0.5, rely=0.5, anchor="center")
        
        # Add padding to login frame
        inner_frame = tk.Frame(login_frame, bg="#2b2b2b")
        inner_frame.pack(padx=40, pady=40)
        
        tk.Label(inner_frame, text="Machine Tool Management System", 
                font=("Arial", 20, "bold"), bg="#2b2b2b", fg="white").pack(pady=(0, 30))
        
        # Username selection frame
        username_frame = tk.Frame(inner_frame, bg="#2b2b2b")
        username_frame.pack(pady=10)
        
        tk.Label(username_frame, text="Username:", font=("Arial", 14), bg="#2b2b2b", fg="white").pack()
        ...
        self.username_dropdown = ttk.Combobox(username_frame, textvariable=self.username_var, 
                                            values=user_list, width=22, state="readonly",
                                            style='Dark.TCombobox', font=("Arial", 11))
        ...
        # Manual entry option
        manual_frame = tk.Frame(inner_frame, bg="#2b2b2b")
        ...
        self.username_entry = tk.Entry(manual_frame, width=24, font=("Arial", 11),
                                      bg="#404040", fg="white", insertbackground="white")
        # Initially hidden
        
        # Password frame
        password_frame = tk.Frame(inner_frame, bg="#2b2b2b")
        password_frame.pack(pady=10)
        
        tk.Label(password_frame, text="Password:", font=("Arial", 14), bg="#2b2b2b", fg="white").pack()
        self.password_entry = tk.Entry(password_frame, width=24, show="*", font=("Arial", 11),
                                      bg="#404040", fg="white", insertbackground="white")
    ...
    def show_registration(self):
        ...
        reg_frame = tk.Frame(main_frame, bg="#2b2b2b", relief="raised", bd=2)
        reg_frame.place(relx=0.5, rely=0.5, anchor="center")
        
        # Add padding
        inner_frame = tk.Frame(reg_frame, bg="#2b2b2b")
        inner_frame.pack(padx=40, pady=40)
        ...
        # Username
        tk.Label(inner_frame, text="Username:", font=("Arial", 14), bg="#2b2b2b", fg="white").pack(pady=5)
        self.reg_username_entry = tk.Entry(inner_frame, width=27, font=("Arial", 11),
                                          bg="#404040", fg="white", insertbackground="white")
        self.reg_username_entry.pack(pady=5)
        
        # Password
        tk.Label(inner_frame, text="Password:", font=("Arial", 14), bg="#2b2b2b", fg="white").pack(pady=5)
        self.reg_password_entry = tk.Entry(inner_frame, width=27, show="*", font=("Arial", 11),
                                          bg="#404040", fg="white", insertbackground="white")
        self.reg_password_entry.pack(pady=5)
        
        # Confirm Password
        tk.Label(inner_frame, text="Confirm Password:", font=("Arial", 14), bg="#2b2b2b", fg="white").pack(pady=5)
        self.reg_confirm_entry = tk.Entry(inner_frame, width=27, show="*", font=("Arial", 11),
                                         bg="#404040", fg="white", insertbackground="white")
        self.reg_confirm_entry.pack(pady=5)
        
        # Role info (operator only now)
        tk.Label(inner_frame, text="Role: Operator (Managers are assigned by Admin)", 
                font=("Arial", 12), fg="lightgray", bg="#2b2b2b").pack(pady=20)
        
        # Buttons
        btn_frame = tk.Frame(inner_frame, bg="#2b2b2b")
        ...
        back_btn = tk.Button(btn_frame, text="Back to Login", command=self.show_login_screen, 
                            bg="#9E9E9E", fg="white", width=15, font=("Arial", 12, "bold"))
    ...
    def create_account(self):
        ...
        role = "operator"  # Always operator now
    ...
    def show_machine_selection(self):
        ...
        tk.Label(header_frame, text=welcome_text, 
                font=("Arial", 18, "bold"), bg="#1a1a1a", fg="white").pack(side="left")
        ...
        tk.Label(main_frame, text="Select a Machine:", 
                font=("Arial", 16, "bold"), bg="#1a1a1a", fg="white").pack(pady=(0, 20))
    ...
    def show_user_management(self):
        ...
        tk.Label(header_frame, text="User Management", 
                font=("Arial", 18, "bold"), bg="#1a1a1a", fg="white").pack(side="left", padx=20)
        ...
        if self.current_user_role == "admin":
            ...
            promote_btn = tk.Button(btn_frame, text="Promote to Manager", 
                                   command=self.promote_user,
                                   bg="#9C27B0", fg="white", font=("Arial", 11, "bold"))
            promote_btn.pack(side="left", padx=(0, 10))
            
            demote_btn = tk.Button(btn_frame, text="Demote to Operator", 
                                  command=self.demote_user,
                                  bg="#607D8B", fg="white", font=("Arial", 11, "bold"))
            demote_btn.pack(side="left", padx=(0, 10))
        ...
        # Configure ttk treeview for dark theme
        style = ttk.Style()
        ...
        # User list table
        table_frame = tk.Frame(main_frame, bg="#2b2b2b", relief="sunken", bd=1)
        ...
        style.configure("Dark.Treeview", font=("Arial", 12))
        style.configure("Dark.Treeview.Heading", font=("Arial", 12, "bold"))
        ...
        # Add scrollbars with dark theme
    ...
    def promote_user(self):
        """Promote selected user to manager (Admin only)"""
        if self.current_user_role != "admin":
            messagebox.showerror("Access Denied", "Only admins can promote users")
            return
        ...
        if messagebox.askyesno("Confirm Promotion", f"Promote '{username}' to Manager role?"):
            users = self.load_users()
            if username in users:
                users[username]["role"] = "manager"
                self.save_users(users)
                self.refresh_user_table()
                messagebox.showinfo("Success", f"'{username}' has been promoted to Manager")
    
    def demote_user(self):
        """Demote selected user to operator (Admin only)"""
        if self.current_user_role != "admin":
            messagebox.showerror("Access Denied", "Only admins can demote users")
            return
        ...
        if messagebox.askyesno("Confirm Demotion", f"Demote '{username}' to Operator role?"):
            users = self.load_users()
            if username in users:
                users[username]["role"] = "operator"
                self.save_users(users)
                self.refresh_user_table()
                messagebox.showinfo("Success", f"'{username}' has been demoted to Operator")
    ...
    def select_machine(self, machine):
        """Handle machine selection and show appropriate interface"""
        self.current_machine = machine
        if machine in self.lens_only_machines:
            self.show_lens_only_interface()
        else:
            self.show_tool_table()
    
    def show_lens_only_interface(self):
        """Show lens-only interface for 5030-1 and 5030-2"""
        self.clear_screen()
        self.root.title(f"Lens Management - {self.current_machine}")
        ...
        tk.Label(header_frame, text=f"Lens Management - {self.current_machine}", 
                font=("Arial", 18, "bold"), bg="#1a1a1a", fg="white").pack(side="left", padx=20)
        ...
        # Center lens change section
        lens_frame = tk.Frame(main_frame, bg="#2b2b2b", relief="raised", bd=2)
        lens_frame.place(relx=0.5, rely=0.5, anchor="center", width=500, height=300)
        
        tk.Label(lens_frame, text="Lens Change Management", 
                font=("Arial", 20, "bold"), bg="#2b2b2b", fg="white").pack(pady=20)
        
        # Get current lens change date
        lens_data = self.load_lens_data()
        current_date = lens_data.get("last_lens_change", "Not Set")
        
        tk.Label(lens_frame, text=f"Last Lens Change Date:", 
                font=("Arial", 14), bg="#2b2b2b", fg="white").pack(pady=10)
        
        self.lens_date_label = tk.Label(lens_frame, text=current_date, 
                                       font=("Arial", 16, "bold"), bg="#2b2b2b", fg="#4CAF50")
        self.lens_date_label.pack(pady=10)
        
        # Update lens change button
        update_btn = tk.Button(lens_frame, text="Update Lens Change Date", 
                              command=self.update_lens_change,
                              bg="#FF9800", fg="white", width=20, height=2,
                              font=("Arial", 14, "bold"))
        update_btn.pack(pady=30)
    
    def load_lens_data(self):
        """Load lens change data for the current machine"""
        filename = f"{self.current_machine.replace('-', '_').lower()}_lens.json"
    ...
    def save_lens_data(self, data):
        """Save lens change data for the current machine"""
        filename = f"{self.current_machine.replace('-', '_').lower()}_lens.json"
        filepath = os.path.join(self.data_folder, filename)
        ...
        try:
            ...
            messagebox.showerror("Error", f"Could not save lens data: {e}")
    
    def update_lens_change(self):
        """Update lens change date after confirmation"""
        if messagebox.askyesno("Lens Change Confirmation", "Was lens change made?"):
            new_date = datetime.now().strftime("%m/%d/%Y")
            lens_data = self.load_lens_data()
            lens_data["last_lens_change"] = new_date
            self.save_lens_data(lens_data)
            
            # Update the display
            self.lens_date_label.config(text=new_date)
            messagebox.showinfo("Success", f"Lens change date updated to {new_date}")
    ...
    def save_machine_data(self, data):
        ...
        filepath = self.get_machine_data_file()
        try:
            # Sort data by tool_id in ascending order
            data.sort(key=lambda x: str(x.get("tool_id", "")).upper())
            with open(filepath, 'w') as f:
                json.dump(data, f, indent=2)
        except Exception as e:
    ...
    def show_tool_table(self):
        ...
        # Lens change section for applicable machines
        if self.current_machine in self.lens_machines:
            lens_frame = tk.Frame(main_frame, bg="#2b2b2b", relief="raised", bd=1)
            lens_frame.pack(fill="x", pady=(0, 20))
            
            lens_inner = tk.Frame(lens_frame, bg="#2b2b2b")
            lens_inner.pack(pady=10, padx=20)
            
            # Get lens change date
            lens_data = self.load_lens_data()
            current_date = lens_data.get("last_lens_change", "Not Set")
            
            tk.Label(lens_inner, text=f"Last Lens Change: {current_date}", 
                    font=("Arial", 12, "bold"), bg="#2b2b2b", fg="white").pack(side="left")
            
            update_lens_btn = tk.Button(lens_inner, text="Update Lens Change", 
                                       command=self.update_lens_change_tool_page,
                                       bg="#FF9800", fg="white", font=("Arial", 11, "bold"))
            update_lens_btn.pack(side="right")
        ...
        # Configure ttk treeview for dark theme
        style = ttk.Style()
        style.configure("ToolDark.Treeview",
                       background="#404040",
                       foreground="white",
                       fieldbackground="#404040",
                       borderwidth=1,
                       relief="solid",
                       font=("Arial", 11))
        style.configure("ToolDark.Treeview.Heading",
                       background="#2b2b2b",
                       foreground="white",
                       relief="raised",
                       borderwidth=1,
                       font=("Arial", 12, "bold"))
        style.map("ToolDark.Treeview",
                 background=[('selected', '#0078d4')])
        ...
        # Columns: Tool ID, Description, PTT (inch), PTT (mm), Height, Last Sharpened
        columns = ("tool_id", "description", "ptt_inch", "ptt_mm", "height", "last_sharpened")
        
        self.tree = ttk.Treeview(table_frame, columns=columns, show="headings", 
                                height=20, style="ToolDark.Treeview")
        
        # Define column headings and widths
        self.tree.heading("tool_id", text="Tool ID")
        self.tree.heading("description", text="Tool Description")
        self.tree.heading("ptt_inch", text="PTT (inch)")
        self.tree.heading("ptt_mm", text="PTT (mm)")
        ...
        self.tree.column("ptt_inch", width=120, anchor="center")
        self.tree.column("ptt_mm", width=120, anchor="center")
    ...
    def update_lens_change_tool_page(self):
        """Update lens change date from tool page"""
        if messagebox.askyesno("Lens Change Confirmation", "Was lens change made?"):
            new_date = datetime.now().strftime("%m/%d/%Y")
            lens_data = self.load_lens_data()
            lens_data["last_lens_change"] = new_date
            ...
            messagebox.showinfo("Success", f"Lens change date updated to {new_date}")
            # Refresh the tool table to update the lens change display
            self.show_tool_table()
    ...
    def refresh_table(self):
        ...
        # Load and display data (already sorted in save_machine_data)
    ...
    def add_new_tool(self):
        ...
        if dialog.result:
            ...
            existing_ids = [tool.get("tool_id", "").upper() for tool in data]
            if dialog.result["tool_id"].upper() in existing_ids:
            ...
            self.save_machine_data(data)  # This will auto-sort
            self.refresh_table()
            ...
            try:
                if filename.lower().endswith('.csv'):
                    with open(filename, 'w', newline='', encoding='utf-8') as csvfile:
                        fieldnames = ['tool_id', 'description', 'ptt_inch', 'ptt_mm', 'height', 'last_sharpened']
